...
//std_msgs::Empty movement_done;
//ros::Publisher pub_move_done("/move_done", &movement_done);
...
ROS_CALLBACK(goalposesCb, geometry_msgs::PoseArray, posesfromros)
  ...
  goal.x = goals.poses[0].position.x;
  goal.y = goals.poses[0].position.y;
        
  goto_xy(goal_msg.x, goal_msg.y);

  /*
  for(int i = 0; i < posesfromros.poses_length; i++)
  {
...
//Publisher respavrstring;

//Publisher respavrpose;
//maximus_position::AvrPose avrpose_msg;
//maximus_position::AvrVel avrvel_msg;
...
//    ros.initCommunication();
    ...
    sei();
/*
    avrpose_msg.x = maximus.pos_X;
    avrpose_msg.y = maximus.pos_Y;
    avrpose_msg.theta = maximus.theta;
    avrpose_msg.vx = 0;
    avrpose_msg.vy = 0;
    avrpose_msg.vth = 0;
*/

//    respavrpose = ros.advertise("avrpose");
    nh.initNode();
    //nh.subscribe(sub);
...
//    digitalWrite(13, HIGH);
    ...
    //write_SaberTooth_speed_M1M2(130, 20000, 20000);
...
void write_SaberTooth_speed_M1M2(char addr, signed long speedM1, signed long speedM2)
{
    ...
    //checkSUM1 = (addr + 6 + m1) & 0x7F;
    //checkSUM1 = (addr + 6 + 96) & 0x7F;
    //checkSUM2 = (addr + 7 + m2) & 0x7F;

    // Motor 1
    //Serial2.print(addr, BYTE);
    //Serial2.print(6, BYTE);
    Serial2.print(m1 , BYTE);
    //Serial2.print(checkSUM1, BYTE);

    // Motor 2
    //Serial2.print(addr, BYTE);
    //Serial2.print(7, BYTE);
    Serial2.print(m2 , BYTE);
    //Serial2.print(checkSUM2, BYTE);
...
//            && (abs(errDif) < 3)) {                        // 2 before
        if ((cmd->state == PROCESSING_COMMAND) && (abs(err) < 40)) {                        // 2 before
...
//            && (abs(errDif) < 0.005)) {                        // 2 before
        if ((cmd->state == PROCESSING_COMMAND) && (abs(err) < 80)) {                        // 2 before
            cmd->state = COMMAND_DONE;
